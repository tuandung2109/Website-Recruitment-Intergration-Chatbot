...
const mockEvaluations = [
    {
        ...
        detailed_analysis: "Ứng viên có nền tảng rất tốt về tài chính với 5 năm kinh nghiệm thực tế. Kỹ năng SQL và phân tích dữ liệu rất phù hợp với yêu cầu công việc xử lý hồ sơ vay. Điểm nổi bật là có chứng chỉ CFA Level 2 và kinh nghiệm tại ngân hàng lớn. Tuy nhiên, cần đánh giá kỹ hơn về khả năng thích nghi với môi trường làm việc mới và văn hóa công ty trong buổi phỏng vấn."
    },
    {
        candidate_id: 2,
        ...
        detailed_analysis: "Ứng viên có nền tảng học vấn tốt và kinh nghiệm cơ bản phù hợp. Điểm mạnh là đã quen với môi trường làm việc tại Đà Nẵng. Tuy nhiên, kỹ năng SQL cần được cải thiện để đáp ứng tốt yêu cầu công việc. Đây là ứng viên tiềm năng nếu sẵn sàng học hỏi và phát triển."
    },
    {
        candidate_id: 3,
        ...
        detailed_analysis: "Ứng viên có nền tảng cơ bản nhưng còn thiếu nhiều kỹ năng quan trọng cho vị trí này. Kinh nghiệm chưa đủ mạnh và thiếu các kỹ năng kỹ thuật như SQL. Có thể cân nhắc cho vị trí entry-level hoặc đào tạo thêm trước khi nhận vào vị trí chính thức."
    },
    {
        candidate_id: 4,
        ...
        gaps: [
            "Cần thương lượng về mức lương",
            "Xác nhận khả năng relocation"
        ],
        interview_focus: [
        ...
        detailed_analysis: "Ứng viên rất xuất sắc với 7 năm kinh nghiệm thực tế tại các ngân hàng lớn. Có đầy đủ kỹ năng kỹ thuật và chứng chỉ chuyên môn. Thách thức duy nhất là mức lương mong muốn cao hơn và cần thuyết phục về việc relocation. Đây là ứng viên top priority cần được ưu tiên phỏng vấn và tạo package hấp dẫn."
    },
    {
        candidate_id: 5,
        ...
        detailed_analysis: "Ứng viên không phù hợp với vị trí Nhân viên tín dụng. Profile và kinh nghiệm không liên quan đến lĩnh vực tài chính ngân hàng. Thiếu hoàn toàn các kỹ năng kỹ thuật và kinh nghiệm cần thiết. Không nên mời phỏng vấn để tránh lãng phí thời gian của cả hai bên."
    }
];
...
const CandidateEvaluation = () => {
    ...
    const getSalaryFitBadge = (salaryFit) => {
        const badges = {
            'Match': { text: 'Phù hợp', color: '#10b981', icon: <CheckCircle size={16} /> },
            'Above': { text: 'Cao hơn', color: '#f59e0b', icon: <AlertCircle size={16} /> },
            'Below': { text: 'Thấp hơn', color: '#ef4444', icon: <XCircle size={16} /> }
        };
    ...
    const getStatistics = () => {
        ...
        const highlyRecommended = evaluations.filter(e => e.recommendation === 'Highly Recommended').length;
        const recommended = evaluations.filter(e => e.recommendation === 'Recommended').length;
        const consider = evaluations.filter(e => e.recommendation === 'Consider').length;
        const notRecommended = evaluations.filter(e => e.recommendation === 'Not Recommended').length;
        
        const avgScore = total > 0 
            ? (evaluations.reduce((sum, e) => sum + (e.overall_score || 0), 0) / total).toFixed(1)
            : 0;
        
        return { total, highlyRecommended, recommended, consider, notRecommended, avgScore };
    };
            ...
            <div className="filters-section">
                <div className="filter-group">
                    ...
                    >
                        ...
                        <option value="highly-recommended">Rất phù hợp ({stats.highlyRecommended})</option>
                        <option value="recommended">Phù hợp ({stats.recommended})</option>
                        <option value="consider">Cân nhắc ({stats.consider})</option>
                        <option value="not-recommended">Không phù hợp ({stats.notRecommended})</option>
                    </select>
                </div>
                        ...
                        <div key={index} className="evaluation-card">
                            ...
                            <div className="evaluation-card-header">
                                <div className="candidate-info">
                                    ...
                                    <div className="candidate-details">
                                        ...
                                        <div className="candidate-meta">
                                            <span><Mail className="meta-icon" />{evaluation.candidate_email}</span>
                                            <span><Phone className="meta-icon" />{evaluation.candidate_phone}</span>
                                            <span><Clock className="meta-icon" />
                                ...
                                <div className="evaluation-score-section">
                                    <div className={`overall-score ${getScoreColor(evaluation.overall_score)}`}>
                                        <span className="score-value">{evaluation.overall_score}</span>
                                        <span className="score-max">/100</span>
                                    </div>
                                    <div className={`recommendation-badge ${getRecommendationColor(evaluation.recommendation)}`}>
                                        {getRecommendationIcon(evaluation.recommendation)}
                                        <span>{evaluation.recommendation}</span>
                                    </div>
                                    <div className="salary-fit-badge" style={{ 
                            ...
                            <div className="scores-breakdown">
                                <div className="score-item">
                                    ...
                                    <div className="score-bar-container">
                                        <div className="score-bar">
                                            <div 
                                                className={`score-bar-fill ${getScoreColor(evaluation.scores?.skills_match)}`}
                                                style={{ width: `${evaluation.scores?.skills_match}%` }}
                                            ></div>
                                        </div>
                                        <span className="score-number">{evaluation.scores?.skills_match}%</span>
                                    </div>
                                </div>
                                ...
                                <div className="score-item">
                                    ...
                                    <div className="score-bar-container">
                                        <div className="score-bar">
                                            <div 
                                                className={`score-bar-fill ${getScoreColor(evaluation.scores?.experience_match)}`}
                                                style={{ width: `${evaluation.scores?.experience_match}%` }}
                                            ></div>
                                        </div>
                                        <span className="score-number">{evaluation.scores?.experience_match}%</span>
                                    </div>
                                </div>
                                ...
                                <div className="score-item">
                                    ...
                                    <div className="score-bar-container">
                                        <div className="score-bar">
                                            <div 
                                                className={`score-bar-fill ${getScoreColor(evaluation.scores?.education_match)}`}
                                                style={{ width: `${evaluation.scores?.education_match}%` }}
                                            ></div>
                                        </div>
                                        <span className="score-number">{evaluation.scores?.education_match}%</span>
                                    </div>
                                </div>
                                ...
                                <div className="score-item">
                                    ...
                                    <div className="score-bar-container">
                                        <div className="score-bar">
                                            <div 
                                                className={`score-bar-fill ${getScoreColor(evaluation.scores?.cultural_fit)}`}
                                                style={{ width: `${evaluation.scores?.cultural_fit}%` }}
                                            ></div>
                                        </div>
                                        <span className="score-number">{evaluation.scores?.cultural_fit}%</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="evaluation-analysis">
                                <div className="analysis-grid">
                                    ...
                                    {/* Interview Focus */}
                                    {evaluation.interview_focus && evaluation.interview_focus.length > 0 && (
